...
class TestCRUDGame:
    ...
    def test_get_current_game_data_return_state(
        ...
        with switch_db(model_game.CurrentGameData, 'test-db-alias') as CurrentGameData:
            ...
            assert state.game_decks.group_deck.deck_top == [], 'wrong group top'
            assert state.game_decks.group_deck.deck_bottom == [], 'wrong group bottom'
            assert state.game_decks.group_deck.pile == [], 'wrong group pile'
            assert state.game_decks.objective_deck.deck_len == 21, \
                'wrong objective len'
            assert state.game_decks.objective_deck.deck_top == [], \
                'wrong objective top'
            assert state.game_decks.objective_deck.deck_bottom == [], \
