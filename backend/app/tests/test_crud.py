...
class TestCRUDGame:
    ...
    def test_get_current_game_data_return_state(
        ...
        with switch_db(model_game.CurrentGameData, 'test-db-alias') as CurrentGameData:
            ...
            assert not state.players[1].user.login, 'wrong user'
    ...
    #     assert player.login is None, 'wrong player'
    ...
    #     def mockreturn(*args, **kwargs) -> Callable:
    #         if args[0]:
    #             return schema_user.UserCurrentData(login=args[0])
    #         else:
    #             return
    ...
    #     assert not player.user.login, 'wrong user'
    ...
    def test_create_new_game(
        ...
        with switch_db(model_game.CurrentGameData, 'test-db-alias') as CurrentGameData:
            game = crud_game.CRUDGame(CurrentGameData, schema_game.CurrentGameData)
            state0 = game.get_current_game_data('DonaldTrump')
            game.create_new_game('DonaldTrump')
            state1 = game.get_current_game_data('DonaldTrump')
            assert state0.id != state1.id, 'not current'
